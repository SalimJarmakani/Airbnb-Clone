...
const { authenticateToken, authorizeRole, checkApiKey } = require(path.join(
  __dirname,
  "..",
  "middlewares",
  "auth"
...
router.get("/AirBnBs", checkApiKey, async (req, res) => {
  try {
...
router.get("/AirBnBs/:id", checkApiKey, async (req, res) => {
  try {
